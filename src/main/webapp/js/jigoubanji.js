function queryList(){
	...
	$.ajax({
		ansyc:false,
		type:"post",
		url:basePath+"organization/baobiaoList",
		dataType:"json",
		data:json,
		success:function(result){
			var count=result.data.count;
			var list=result.data.classList;
...
function Export(){
	window.location.href=basePath+"/organization/Exportbaobiao.do?organization="+$("#organization").val()+"&page="+parseInt($("#currentPage").html())+"&rows="+parseInt($("#pageSize").val());
};


function myPrint(){
	$("#classTable").attr("border","1px");
    var newWindow=window.open("打印窗口","_blank");
    var table=document.getElementById("classReport");
    var docStr =table.innerHTML;
    newWindow.document.write(docStr);
    newWindow.document.close();
    newWindow.print();
    $("#classTable").attr("border","0px");
    newWindow.close();
}